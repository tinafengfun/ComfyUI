...
| 2. Workflow inventory | Understand graph shape, branches, outputs, and critical paths. | Topology inventory, branch map, executable-node list, node inventory table. | Graph cannot be represented or output branches are unclear. |
| 3. Asset and custom-node prep | Resolve models, LoRAs, input media, custom nodes, nested repos, then acquire source-known dependencies into an isolated workflow cache when approved. | Asset ledger, custom-node ledger, acquisition log, missing/proprietary list. | Required source-identical asset or critical custom-node source is unavailable and no approved smoke-only alias is acceptable. |
...
| 2 | Workflow inventory | `{workflow_slug}/02-inventory.md`, or split as `{workflow_slug}/02-workflow-topology.md` plus `{workflow_slug}/02-node-inventory.csv` | `node_count`, `link_count`, `outputs`, `branches`, `critical_path`, `custom_node_packages`, `export_risks`, `disconnected_nodes`, `node_inventory` |
| 3 | Asset/custom-node ledger and acquisition log | `{workflow_slug}/03-assets.csv`, `{workflow_slug}/03-custom-nodes.md`, and when downloads/clones occur `{workflow_slug}/03-acquisition-log.md` | `requested_name`, `resolved_path`, `source`, `state`, `staged_path`, `repo`, `commit`, `custom_node_cache_path`, `wrapper_source_evidence`, `install_status`, `acquisition_status`, `mirror_used`, `credential_recorded`, `remaining_hard_stops` |
...
When Step 0 or Step 1 finds a hard stop whose source is known but not staged, Step 3 must include a bounded acquisition pass before environment deployment. Download or copy exact model files into an isolated workflow cache and clone custom-node repositories there first. Also inspect selected custom-node wrapper code for hidden runtime assets and package-specific cache paths. This does not prove runtime support; it only moves dependencies from `source reachable but not staged` to `staged`. Installation, ComfyUI restart, source audit, and prompt validation remain separate gates.

If a step is run out of the ideal order or repeated after new evidence appears, refresh its dependency-state inputs before writing conclusions. For example, if Step 3 has already staged assets or approved a replacement input before Step 2 is finalized, Step 2 must consume the latest `03-assets.csv`, `03-custom-nodes.md`, and `03-acquisition-log.md` state instead of repeating stale Step 0 hard stops.
...
18. Do not consider Step 3 complete from workflow-visible selectors alone. Selected custom nodes may introduce hidden runtime assets through wrapper defaults and auto-download calls.
...
8. Zimage showed that Step 2 artifact naming must be explicit: `02-inventory.md` can be a single report, but complex workflows may need a split topology report plus a node inventory CSV.
...
21. Zimage Step 7 showed that Step 3 must inspect custom-node wrapper defaults and auto-download code: `AIO_Preprocessor` hid `depth_anything_v2_vitl.pth` behind `DepthAnythingV2Preprocessor` and `hf_hub_download()`, so a workflow-visible asset scan missed it.
