...
| 新增/变更 | 评价 |
|---|---|
| ✅ 新增 `migration-workflow/README.md` 作为执行入口 | **很大提升。** 终于有了那个"一页纸操作手册"——11 步 operating map 含 prompt/skill 引用、required outputhuman intervention point，一目了然。 |
...
| ✅ `skills/01-feasibility-analysis-skill.md` 加入了 hardware reference 引用 | 交叉引用到位。 |
...
这个做法 **在文档严谨性上是正确的**——它避免了凭空编造 spec 带来的误导。但从你的需求"更容易评估筛选平台"）来看，**当前版本还差半步**。具体分析如下：

### 👍 做得好的部分

1. **"Required hardware fields" 表**（L20-34）非常实用，给出了必须采集的字段和获取命令
2. **"Capacity routing worksheet"**（L39-45）用 Dasiwa 实际证据给出了路由判断，比我原来建议的"猜测式 spec 表"更诚实
...
当前版本的问题是：一个新工程师拿到文档后，在实测 **之前** 没有任何参点来做初步筛选。比如拿到一个新 workflow，想判断 "应该去 B60 上试还是直接找更大的卡"，除了 "24 GB-class" 这个从 Dasiwa 来的标签，没有其他信息。

**建议**：在 `intel-xpu-hardware-reference.md` 中增一个 **"Public spec reference"** 段（与"从迁移证据中得到的"分开），明确标注这些是公开信息而非实测值：
...
### 2. B60 的 VRAM 与公开信息存在潜在矛盾

文档中反复说 "B60 / 24 GB"。但公开 Intel 产品线中，Arc B580 是 12GB GDDR6。如果 "B60" 是一个内部环境名（比如某个服务器上的卡），那这是合理的。但如果有人把它理解为 Arc B-系列消费卡，就会困惑。
