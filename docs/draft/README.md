...
Each logical migration step should be used with the prompt and skill mapped in the generated draft set below. Some prompt/skill filenames still carry their historical draft number, but the logical step order is authoritative. The prompt is the task instruction; the skill is the execution reference. If a rule is updated, update both only when the user-facing instruction and the execution method both change.

Important numbering rule: **logical Step 01 is asset and custom-node resolution**. The files `prompts/01-feasibility-analysis-prompt.md` and `skills/01-feasibility-analysis-skill.md` keep their legacy filename numbers, but they are logical Step 02 in this flow. Do not use the feasibility prompt/skill for asset preparation.
