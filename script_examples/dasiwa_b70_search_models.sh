...
for name in basenames:
    for label, api_template, web_template in sources:
        print(f"=== {label}: {name} ===")
        cmd = ["curl", "-L", "--silent", "--show-error", api_template.format(term=name)]
        try:
...
for name in "${BASENAMES[@]}"; do
  ...
  query="$(urlencode "${name%.safetensors}")"
  echo "fallback: https://www.modelscope.cn/models?name=${query}"
  echo "fallback: https://www.modelscope.cn/search?search=${query}"
  echo
done
